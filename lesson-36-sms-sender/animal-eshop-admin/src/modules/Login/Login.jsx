...
import { login, loginWithGoogle, finishLogin } from "../../redux/auth/auth-thunks";
import { verifyUserApi, getGoogleLoginLinkApi } from "../../shared/api/auth-api";

const Login = () => {
  const [searchParams, setSearcParams] = useSearchParams();
  const [successVerify, setSuccessVerify] = useState(false);
  const {loading, error, otp} = useSelector(selectAuth);
  ...
  return (
    ...
    >
      ...
      {!otp && <LoginForm submitForm={submitForm} loading={loading} />}
      {otp && <OTPForm submitForm={onFinishLogin} />}
